...
"""Build the KCC Game Makers print pack.
    ...
    python3 build_print.py <repo-root> <outputs-dir>
...
def strip_front(md):
    ...
    for i, l in enumerate(lines):
        s = l.strip()
        # Leading headings are the page header's job; skip them even when
        # separated by blank lines.
        if not "".join(out).strip() and (s.startswith("# ") or s.startswith("## ")):
            continue
...
def week_dir(n):
    """Folder for everything a volunteer needs on the night of week n."""
    return os.path.join("Weeks", "Week %d - %s" % (n, WEEK_TITLES[n]))


def jobs(root):
    """Every PDF in the pack. `out` is relative to the outputs folder, which is
    laid out exactly as it should appear in Google Drive."""
    J = []
    C = os.path.join(root, "course")
    A = os.path.join(root, "admin")
    for n in range(1, 9):
        wd = week_dir(n)
        lesson = [f for f in sorted(os.listdir(os.path.join(C, "lessons")))
                  if f.startswith("%02d-" % n)][0]
        J.append(dict(out=os.path.join(wd, "Week %d - Lesson Plan.pdf" % n),
                      src=os.path.join(C, "lessons", lesson), kind="doc",
                      title="Week %d Lesson Plan" % n, sub=WEEK_TITLES[n],
                      badge="", doc="Week %d lesson plan" % n))
        J.append(dict(out=os.path.join(wd, "Week %d - Handout.pdf" % n),
                      src=os.path.join(C, "handouts", "class-%d.md" % n), kind="student",
    ...
    V, AD, S = "Volunteers", "Admin", "Showcase"
    J += [
        dict(out=os.path.join(V, "Programme Overview.pdf"), src=os.path.join(C, "program-overview.md"),
             kind="doc", title="Programme Overview",
             sub="How the club runs, and why", badge="", doc="Programme overview"),
        dict(out=os.path.join(V, "Volunteer Briefing.pdf"), src=os.path.join(A, "volunteer-briefing.md"),
             kind="doc", title="Volunteer Briefing", sub="Read before week one",
             badge="", doc="Volunteer briefing"),
        dict(out=os.path.join(V, "Long Project - Sky Catcher.pdf"),
             src=os.path.join(C, "long-project-sky-catcher.md"),
             kind="doc", title="The Long Project", sub="Sky Catcher, week by week",
             badge="", doc="Long project guide"),
        dict(out=os.path.join(V, "Concept Map.pdf"), src=os.path.join(C, "concept-map.md"),
             kind="doc", title="Concept Map", sub="What is taught when",
             badge="", doc="Concept map"),
        dict(out=os.path.join(V, "External Resources.pdf"), src=os.path.join(C, "resources.md"),
             kind="doc", title="External Resources", sub="Free material for extension and catch-up",
             badge="", doc="External resources"),
        dict(out=os.path.join(AD, "Parent Information Letter.pdf"),
             src=os.path.join(A, "parent-info-letter.md"),
             kind="doc", title="Parent Information", sub="",
             badge="", doc="Parent letter"),
        dict(out=os.path.join(AD, "Consent and Photo Permission.pdf"),
             src=os.path.join(A, "consent-and-photo-permission.md"),
             kind="doc", title="Consent and Photo Permission",
             sub="Return before the first session", badge="", doc="Consent form"),
        dict(out=os.path.join(AD, "Attendance Tracker.pdf"), src=os.path.join(A, "attendance-tracker.md"),
        ...
        dict(out=os.path.join(AD, "Pre-launch Checklist.pdf"), src=os.path.join(A, "pre-launch-checklist.md"),
             kind="doc", title="Pre-launch Checklist", sub="Start six weeks out",
             badge="", doc="Pre-launch checklist"),
        dict(out=os.path.join(AD, "Venue and Device Checklist.pdf"),
             src=os.path.join(A, "venue-device-checklist.md"),
             kind="doc", title="Venue and Device Checklist", sub="Before you commit to a room",
             badge="", doc="Venue checklist"),
        dict(out=os.path.join(AD, "Scratch Accounts Setup.pdf"),
             src=os.path.join(A, "scratch-accounts-setup.md"),
             kind="doc", title="Scratch Accounts", sub="Set up six weeks out",
             badge="", doc="Scratch accounts guide"),
        dict(out=os.path.join(S, "Showcase Run Sheet.pdf"), src=os.path.join(A, "showcase-run-sheet.md"),
...
CERT_PDF = os.path.join("Showcase", "Certificate Template.pdf")
CERT_HTML = os.path.join("Showcase", "Certificate Template.html")
...
async def render(root, outdir):
    ...
    async with async_playwright() as pw:
        ...
        for j in jobs(root):
            ...
            os.makedirs(os.path.dirname(os.path.join(outdir, j["out"])), exist_ok=True)
        ...
        await pg.emulate_media(media="print")
        os.makedirs(os.path.join(outdir, "Showcase"), exist_ok=True)
        await pg.pdf(path=os.path.join(outdir, CERT_PDF),
                     format="A4", landscape=True, print_background=True,
                     margin={"top": "0", "bottom": "0", "left": "0", "right": "0"})
        built.append(CERT_PDF)
        await browser.close()
    return built
...
if __name__ == "__main__":
    ...
    outdir = sys.argv[2] if len(sys.argv) > 2 else os.path.join(root, "outputs")
    made = asyncio.run(render(root, outdir))
    io.open(os.path.join(outdir, CERT_HTML), "w",
